...
Alarm::CallBack() 
{
    ...
    bool woken = _sleeplist.put_to_ready();
...
void sleep_list::put_to_sleep(Thread*t, int x) {
    ASSERT(kernel->interrupt->getLevel() == IntOff);
    _threadlist.push_back(sleep_thread(t, _current_interrupt + x));
    t->Sleep(false);
}

bool sleep_list::put_to_ready() {
    bool woken = false;

    _current_interrupt ++;

    for(std::list<sleep_thread>::iterator it = _threadlist.begin(); 
        it != _threadlist.end(); ) {
        if(_current_interrupt >= it->when) {
            woken = true;
//            cout << "sleep_list::put_to_ready Thread woken" << endl;
