          ...
          while(kernel->machine->usedPhyPage[j]!=FALSE&&j<NumPhysPages)
              j += 1;                            // find valid frame space
                 ...
                 if( j < NumPhysPages){          // if find valid frame space, save the page in virtual memory into physical memory
                      ...
                      kernel->Swap_Space->ReadSector(pageTable[vpn].virtualPage, buffer);      // read data from swap area
                     ...
                     for(int cc=0;cc<32;cc++){
                             if(min > pageTable[cc].count){
                                    min = pageTable[cc].count;
                                    Swap_out_page = cc;
                                    
                             }
                     } 
                     pageTable[Swap_out_page].count++;  
                     
                     //printf("Page%d swap out!\n",Swap_out_page);
                     bcopy(&mainMemory[Swap_out_page*PageSize],buffer1,PageSize);
                     kernel->Swap_Space->ReadSector(pageTable[vpn].virtualPage, buffer2);
                     bcopy(buffer2,&mainMemory[Swap_out_page*PageSize],PageSize);
                     kernel->Swap_Space->WriteSector(pageTable[vpn].virtualPage,buffer1);
